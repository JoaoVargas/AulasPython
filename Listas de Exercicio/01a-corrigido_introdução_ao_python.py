...
# resolva utilizando o que aprendeu nas aulas, se não lembrar não tem problema pesquisar no google (essa é a maior parte da programação).
...
def dias_para_segundos(dias, horas, minutos, segundos):
    ...
    """
    totalSegundos = (((((dias*24)+horas)*60)+minutos)*60)+segundos
    print(totalSegundos)
...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    """
    cigarrosFumados = (cigarros_fumados_por_dia * 365) * anos_fumando
...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    ...
    """
    parcial1 = ((p1 * 7) + (ep1 * 3)) / 10
    parcial2 = ((p2 * 7) + (ep2 * 3)) / 10
    geral = (parcial1 + parcial2) / 2
    passou = geral >= 7
    print(passou)
...
def salario(valor_hora, horas_mensais):
    ...
    """
    salarioMensalBruto = valor_hora * horas_mensais
    salarioMensalLiquido = salarioMensalBruto - (salarioMensalBruto*0.24)
    print(salarioMensalLiquido)
    
    return(salarioMensalLiquido)
...
def tinta(metros_pintar):
    ...
    """
    metrosPorLata = 18 * 3
    latas = round((metros_pintar / metrosPorLata) + 0.5)
    print(latas)
