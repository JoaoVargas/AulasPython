...
def data_valida(data):
    ...
    """
    ano = (int(data[6])*1000) + (int(data[7])*100) + (int(data[8])*10) + (int(data[9])*1)
    mes = (int(data[3])*10)   + (int(data[4])*1)
    dia = (int(data[0])*10)   + (int(data[1])*1)
    bissexto = False
    numDias = 0
    ...
    if (ano > 0) and (mes > 0 and mes <= 12) and (dia > 0 and dia <= numDias):
        return True
    else:
        return False
